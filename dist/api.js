...
const server = createServer(async (req, res) => {
    ...
    if (resolved.session)
        sessionManager.touch(resolved.session.id);
    try {
        ...
        if (method === "POST" && url.pathname === "/v1/actions/navigate") {
            ...
            if (sessionError) {
                json(res, sessionError.status, sessionError.body);
                return;
            }
            const t0 = Date.now();
        ...
        if (method === "POST" && url.pathname === "/v1/intercept") {
            ...
            const page = pages[0] || await browser.newPage();
            if (body.enabled) {
                const blockPatterns = (body.blockPatterns || []).filter((p) => typeof p === "string" && p.length > 0);
        ...
        if (method === "POST" && url.pathname === "/v1/actions/upload") {
            ...
            if (!dir) {
                json(res, 500, { error: "No temp directory available" });
                return;
            }
            ...
            try {
                await withBrowser(session.browserProcess.wsEndpoint, async (page) => {
                    const input = await page.$(body.selector);
                    if (!input)
                        throw new Error(`Element not found: ${body.selector}`);
        ...
        if (method === "GET" && downloadMatch) {
            ...
            const dir = session.browserProcess.downloadDir;
            if (!dir) {
                json(res, 404, { error: "No download directory" });
                return;
            }
...
function recordAction(sessionId, action, params, durationMs) {
