...
interface Histogram {
  buckets: number[]; // cumulative counts aligned to BUCKETS_MS, plus +Inf at the end
  count: number;
  sumMs: number;
}
...
export function recordRequest(method: string, pathname: string, status: number, durationMs: number): void {
  ...
  for (let j = i; j < h.buckets.length; j++) h.buckets[j]++;
...
function percentile(h: Histogram, p: number): number {
  ...
  for (let i = 0; i < BUCKETS_MS.length; i++) {
    if (h.buckets[i] >= target) return BUCKETS_MS[i];
  }
...
export function snapshot(): Record<string, EndpointStats> {
